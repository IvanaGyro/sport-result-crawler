...
import fs from 'fs';
import { Severity, Weather, Light, RoadHierarchy, RoadGeometry, Position, RoadMaterial, RoadSurfaceWet, RoadSurfaceDefect, Obstacle, SightDistance, TrafficSignal, TrafficSignalStatus, DirectionDivider, NormalLaneDivider, FastSlowLaneDivider, EdgeLine, CrashType, } from '../Case';
import Vehicle, { VehicleUsage } from '../Vehicle';
import { Gender, InjurySeverity, InjuriedArea, SaftyDevice, Smartphone, Action, DriverQualification, License, DrunkDriving, CrashArea, Cause, Job, TravelPurpose, Citizenship, } from '../Party';
const maxPartyCount = 5;
...
function weatherToString(weather) {
    const mapping = {
        [Weather.STROM]: '暴雨 ',
...
function roadHierarchyToString(roadHierarchy) {
    const mapping = {
...
function roadGeometryToString(roadGeometry) {
    const mapping = {
...
function roadMaterialToString(roadMaterial) {
    const mapping = {
...
function roadSurfaceWetToString(roadSurfaceWet) {
    const mapping = {
...
function roadSurfaceDefectToString(roadSurfaceDefect) {
    const mapping = {
...
function obstacleToString(obstacle) {
    const mapping = {
...
function sightDistanceToString(sightDistance) {
    const mapping = {
...
function trafficSignalToString(trafficSignal) {
    const mapping = {
...
function trafficSignalStatusToString(trafficSignalStatus) {
    const mapping = {
...
function directionDividerToString(directionDivider) {
    const mapping = {
...
function normalLaneDividerToString(normalLaneDivider) {
    const mapping = {
...
function fastSlowLaneDividerToString(fastSlowLaneDivider) {
    const mapping = {
...
function edgeLineToString(edgeLine) {
    const mapping = {
        [EdgeLine.HAVE]: '有',
        [EdgeLine.NONE]: '無',
    };
...
function crashTypeToString(crashType) {
    const mapping = {
...
function vehicleUsageToString(vehicleUsage) {
    const mapping = {
...
function injurySeverityToString(injuryseverity) {
    const mapping = {
...
function injuriedAreaToString(injuriedarea) {
    const mapping = {
...
function saftyDeviceToString(saftydevice) {
    const mapping = {
...
function smartphoneToString(smartphone) {
    const mapping = {
...
function driverQualificationToString(driverqualification) {
    const mapping = {
...
function drunkDrivingToString(drunkdriving) {
    const mapping = {
...
function crashAreaToString(crasharea) {
    const mapping = {
...
function isHitAndRunToString(isHitAndRun) {
...
function travelPurposeToString(travelpurpose) {
    const mapping = {
...
function citizenshipToString(citizenship) {
    const mapping = {
...
export default class CSVExporter {
    static export(filename, cases, partyNumber = 2, combine = false) {
        const toStringStream = transform(cases, (curCase) => {
            const transmformedCase = {
                id: curCase.id,
            ...
            transmformedCase.parties = curCase.parties.map((party) => {
                const { order } = party;
                return {
                    id: party.id,
                    order: `${order}`,
        ...
        if (combine) {
            ...
            const flattenStream = transform((curCase) => {
                const flattenCase = curCase;
                curCase.parties.forEach((party) => {
                    const { order } = party;
                    Object.assign(flattenCase, Object.fromEntries(Object.entries(party).map((key, value) => [`${key}Party${order}`, value])));
                });
            });
            ...
            const caseFileStream = fs.createWriteStream(filename);
            const partyFileStream = fs.createWriteStream(partyFilename);
            ...
            toStringStream
                .on('data', (stringifiedCase) => {
                stringifiedCase.parties.forEach((party) => {
