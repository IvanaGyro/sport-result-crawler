...
function assignIfExists(target, key, value) {
    let cleanValue;
...
function composeAddress(firstAdministrativeLevel, secondAdministrativeLevel, road, section, lane, alley, number) {
...
export default class TPEParser extends BaseParser {
    ...
    async parse(verify = false) {
        this.verify = verify;
        this.cases = [];
        this.caseMapping = {};
        const outStream = this.csvParser.on('headers', (headers) => {
    ...
    parseHackathon(data) {
        ...
        const firstLocation = composeAddress(data.OccurAddr1_1, data.OccurAddr1_2, data['路段一'], data['路段一段'], data['巷'], data['弄'], data['號']);
        if (this.verify) {
            ...
            if (!data['路段'].startsWith(data['路段一'])) {
                throw new Error(`${errorPrefix} First road address is not consistent: "${data['路段一']}" and "${data['路段']}"`);
            }
            if (data['路段2'] !== '段' && !data['路段2'].startsWith(data['路段二'])) {
                throw new Error(`${errorPrefix} Second road address is not consistent: "${data['路段二']}" and "${data['路段2']}"`);
            }
        }
        const secondLocation = composeAddress(data.OccurAddr1_1, data.OccurAddr1_2, data['路段二'], data['路段二段'], data['巷(岔路)'], data['弄(岔路)'], data['號(岔路)']);
