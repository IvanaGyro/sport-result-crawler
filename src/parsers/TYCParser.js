...
function isChineseNumberOrNumber(value) {
    return (['一', '二', '三', '四', '五', '六', '七', '八', '九', '十'].includes(value) || isNumber(value));
}
...
export default class TYCParser extends BaseParser {
    ...
    parseLineBefore2016(data) {
        ...
        if (party.order === 1 && Number(data['主要肇因']) !== party.cause) {
            throw new Error(`${this.filePath}:${this.rowNumber} The cause of the first party is not the same as the main cause.`);
        }
        ...
        if ((currentCase.date !== this.prevCase?.date
            || currentCase.location !== this.prevCase?.location)
            && party.order !== 1) {
            throw new Error(`${this.filePath}:${this.rowNumber} The new case doesn't start with the first party.`);
        }
